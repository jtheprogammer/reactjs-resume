  ...
  return (
    <Navbar expand="lg" sticky="top" className="header" >
      ...
      <Nav.Link as={NavLink} to="/">
        <Navbar.Brand className="header_home" > <CustomButton icon={navData.Home.icon} /> </Navbar.Brand>
      </Nav.Link>
      <Navbar.Toggle />
      <Navbar.Collapse className="navbar_collapse">
        ...
        <div className="header_socials" >
          {Object.keys(navData.socials).map(media => ( <a key={navData.socials[media].text} className="header_icons" href={navData.socials[media].link} target="_blank" rel="noreferrer" >
            {navData.socials[media].icon}
          </a>
          ))}
        </div>

        <CustomButton className="for_hire" text={navData.Button.text} icon={navData.Button.icon} />
      </Navbar.Collapse>
    </Navbar >
  );
};
